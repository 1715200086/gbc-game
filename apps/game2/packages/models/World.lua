...
local RedisService = cc.import("#redis")
local JobService = cc.import("#jobs")
local BeansService = cc.import("#beanstalkd")
...
function World:ctor(connect)
	...
	self.redis_ = self:getInstance():getRedis()
  --  self.redis_:connect()
...
function World:broadcastNetMsg(action, netMsg)
	...
	--这个函数服务器会发消息给所有与他建立socket通道的客户端
end
...
function World:sendMsg(action, args)
	...
	--服务器会和每个客户端建立一个sock连接 sendMessageToSelf 这个函数就是用于在一个socket通道的服务器端发给客户端消息
end
